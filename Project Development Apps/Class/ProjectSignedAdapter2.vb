Imports System.Text
Public Class ProjectSignedAdapter2
    ...
    Private Shared myInstance As ProjectSignedAdapter2
    ...
    Public Shared Function getInstance()
        If myInstance Is Nothing Then
            myInstance = New ProjectSignedAdapter2
        End If
        Return myInstance
    End Function
    ...
    Public Function LoadData() As Boolean Implements IAdapter.LoadData
        ...
        SB.Clear()
        'SB.Append(String.Format("select tx.*,p.projectid as pidname,p.projectname,v.id as vendorid,v.shortname,pp.input_name as projectstage,case phase_status when 1 then 'Passed'  when 2 then 'Failed' end as phase_status_name,pic.username as pic,p.lognumber as plog,p.docreceived as pdoc from pd.projecttx tx left join pd.project p on p.id = tx.projectid left join pd.vendor v on v.id = p.vendorid left join pd.project_phase pp on pp.id = tx.project_phase_id left join pd.user pic on pic.id = p.pic {0};", ProjectCriteria))
        SB.Append(String.Format("select tx.*,p.projectid as pidname,p.projectname,v.id as vendorid,v.shortname,pp.input_name as projectstage,case phase_status when 1 then 'Passed'  when 2 then 'Failed' end as phase_status_name,pic.username as pic,p.lognum as plog,p.yearlognum as pyearlog,p.docreceived as pdoc from pd.projecttx tx left join pd.project p on p.id = tx.projectid left join pd.vendor v on v.id = p.vendorid left join pd.project_phase pp on pp.id = tx.project_phase_id left join pd.user pic on pic.id = p.pic {0};", ProjectCriteria))
        ...
        SB.Append("select * from pd.project_phase where id <= 7 order by order_line;")
        ...
        'Dim uc1(1) As DataColumn
        'uc1(0) = DS.Tables(0).Columns("pyearlog")
        'uc1(1) = DS.Tables(0).Columns("plog")
        'Dim UniqueConstraint1 = New UniqueConstraint("PLog", uc1)
        'DS.Tables(0).Constraints.Add(UniqueConstraint1)

        'Dim uc2(2) As DataColumn
        'uc2(0) = DS.Tables(0).Columns("yearlognum")
        'uc2(1) = DS.Tables(0).Columns("lognum")
        'uc2(2) = DS.Tables(0).Columns("project_phase_id")
        'Dim UniqueConstraint2 = New UniqueConstraint("Lognum", uc2)
        'DS.Tables(0).Constraints.Add(UniqueConstraint2)
    ...
    Public Function Save() As Boolean Implements IAdapter.Save
        ...
        If Not IsNothing(ds2) Then
            ...
            Catch ex As Exception
                Dim myerrmessage = ex.Message
                If myerrmessage.Contains("Backend") Then
                    myerrmessage = "Failed to save to Database. Duplicate Log Number found. Please check."
                End If
                MessageBox.Show(myerrmessage)
                DS.Merge(ds2)
            End Try
        End If
        Return myret
